...
## 4.6. IPv6 直连与 DDNS 域名 (v1.7.19)

### 设计动机

`BaseUrlResolver.IPV6_DIRECT_URL` 常量指向 NAS 的 IPv6 直连地址。中国移动通过 DHCPv6-PD 周期性轮换 `/64` 前缀（路由器重启、ISP 维护、租约到期都会触发），一旦轮换，硬编码的字面 IPv6 地址就失效。

### DDNS 域名方案 (v1.7.19)

v1.7.19 起，`IPV6_DIRECT_URL` 改为 DDNS 域名 `http://nas.feiyemomo.top:8088/`。DDNS 提供商在前缀轮换时自动更新 AAAA 记录，Android 端无需任何动态获取逻辑——每次 probe 时 DNS 解析自然拿到当前地址。

```
IPV6_DIRECT_URL = "http://nas.feiyemomo.top:8088/"
                   │
                   ▼ DNS 查询 AAAA 记录
                   │
                   ▼ 返回当前 NAS IPv6 地址（DDNS 自动跟踪前缀轮换）
```

### v1.6.27 动态获取机制（已移除）

v1.6.27 曾实现过通过 `/api/v1/network/ipv6` 端点动态获取 NAS 出站 IPv6 地址的机制（`fetchDynamicIpv6Url` + `dynamicIpv6Url` + `tokenProvider`）。v1.7.19 移除了该机制，原因：

1. DDNS 域名的 AAAA 记录已由 DDNS 提供商自动跟踪前缀轮换，无需 App 主动查询后端验证
2. `/network/ipv6` 端点需要 JWT（登录前无法调用），而 DDNS 域名无此限制
3. 移除后减少了每次 probe 的网络往返（3s 超时的 GET 请求）

移除的组件：`fetchDynamicIpv6Url()` 函数、`dynamicIpv6Url` 变量、`tokenProvider` lambda、`AppContainer` 中的 `resolver.tokenProvider = ...` 注入。

### `isIpv6Direct()` / `isDirectPath()`
...
WebRTC 代码在 `resolved` 指向 DDNS IPv6 域名时跳过 STUN/TURN 收集（直连路径只需 host candidate）。

### 与后端 `PrefixWatcher` 的协作

后端 `PrefixWatcher` 每 5 分钟检测前缀轮换，轮换时：
1. 发布 EventBus 事件 `network.ipv6.prefix_rotated`
2. 调用 `FrigateClient.SetWebRTCCandidates()` 推送新 candidate 给 go2rtc

Android 端无需任何主动操作——下一次 `probeSync()`（≤5 分钟内）DNS 解析 DDNS 域名时自然拿到新地址。整个恢复过程 <10 分钟，无需人工干预。
