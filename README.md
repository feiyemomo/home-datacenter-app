...
- **Fisheye chip + 下拉倍速菜单 + 全屏按钮右下角（v1.6.4 rev3）**：v1.6.4 rev2 的 chip "连成一片了"——根因是 chip 内容太长（"HH:mm:ss · Ns"）+ padding 过大（10dp/5dp）+ fisheye minScale=0.45 让边缘 chip 文字几乎不可读。rev3 三处改动：① **chip 变窄**——内容从"HH:mm:ss · Ns"缩到"HH:mm"（秒和时长作为 skim 时的噪音去掉），padding 从 10dp/5dp 减到 4dp/2dp，margin 从 4dp 减到 1dp，字号从 11sp 减到 9sp；fisheye `minScale` 从 0.45 提到 0.6（边缘可读+间隙清晰）。② **倍速菜单改回下拉样式**——rev2 的 Material AlertDialog 用户反馈"还是下拉样式好"。改用 `ListPopupWindow`（不是 PopupMenu，因 PopupMenu 在 Android < 29 上回退到 4.x context menu 外观），anchor 到 btnPlaybackSpeed，宽度按最长 label + 48dp padding 测量自适应。点击即应用并 dismiss。③ **全屏按钮移到播放器右下角**——从 `top|end` (marginTop 54dp) 移到 `bottom|end` (margin 6dp)，符合 YouTube/B 站等视频 app 的肌肉记忆位置。
