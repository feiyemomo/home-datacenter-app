...
- **Fisheye chip + 全屏点击收起（v1.6.4）**：v1.6.3 的 chip 滚动列表用户反馈"划 chip 还是太慢，且不直观"。v1.6.4 不再删除 chip 列表，而是给 `HorizontalScrollView` 加上**鱼眼变换**（自定义 `FisheyeChipScroller`）：滚动时每个 chip 根据距离屏幕中心的远近动态缩放——中心 chip 保持原大小（scale=1.0），边缘 chip 缩到 0.55 + alpha 0.5。用户既能"一屏看到多个 chip"（边缘缩小的 chip 提供上下文），又能保持中心的精确可点击区。变换在 `onScrollChanged` 和 `onLayout` 中重算，GPU 开销低（仅 `scaleX/scaleY/alpha`，200 chip 也能 60fps）。同时配套布局重构：① dialog_recordings.xml 改为垂直 LinearLayout——**播放器固定 200dp 在顶部，中部是 fisheye chip scroller 占满剩余空间，底部是 dayScrubBarContainer（SeekBar + AlertRangeOverlay）**——用户原话"正常未全屏时，播放器就放在顶部，然后中部放chip"；② `btnPlaybackSpeed` + `btnFullscreen` 浮在 playerView 顶部 top|end（marginTop 54dp 错开），`btnBack` 在 playerView 底部 center_horizontal，消除 v1.6.3 中按钮与底部 scrub bar 的重叠；③ `PlayerFullscreenHelper` 新增 `controllerSyncViews: List<View>` 参数——RecordingsDialog 把 `dayScrubBarContainer + motionChipScroller + btnBack + btnPlaybackSpeed` 加入此列表，**全屏时点击 playerView 即 toggle 这些 views 的可见性**（实现"全屏时，点击播放器收起进度条等的插件，再次点击是在展现"）。tap-to-toggle 通过 `playerView.setOnClickListener` 直接实现（不依赖 ExoPlayer controller listener，因 host 用 `useController=false`）；用 `controllerOverlayVisible` 布尔字段记录当前状态以便多次进入/退出全屏状态保持。`hideOnFullscreen` 仅保留 `toolbar`（始终隐藏不跟随 toggle），`playerHostFrame` 作为 `secondaryPlayerView` 传入让其在全屏时也变 MATCH_PARENT（否则 playerView 即使设 MATCH_PARENT 仍被 200dp 父容器裁剪）。配套修复：倍速按钮 `formatSpeed` 把 `1.5f` 截断为 `1` 显示成"1x"的 bug（同时与"1.0x"重复）—— 改为 `speed == speed.toInt().toFloat()` 判断整数速度，整数用 `"${speed.toInt()}x"`，非整数用 `"${speed}x"`。
