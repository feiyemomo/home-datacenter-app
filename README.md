...
- **Fisheye chip + 全屏点击收起 + 倍速菜单优化（v1.6.4 rev2）**：v1.6.4 rev1 的 chip 滚动方案用户反馈"边缘 alpha 0.5 看着像隐藏"。rev2 把 `FisheyeChipScroller` 从 HorizontalScrollView 改成 **不滚动的 ViewGroup**——所有 chip 强制 fit 到屏幕宽度内（每个 chip 占 `viewportWidth / N` 等宽 slot），**一屏看到一整天的 chip**，无需滚动；中心 chip scale=1.0 保持原大小，边缘 chip scale=0.45（横向压缩但不透明不隐藏），实现"挤压效果"。计算：`distNorm = abs((i+0.5)/N - 0.5) * 2` → `scale = lerp(1.0, 0.45, distNorm)`。alpha 始终 1.0。布局重构：dialog_recordings.xml 改为 4 段垂直布局——**① 播放器 200dp 在顶部 → ② 进度条（dayScrubBarContainer，SeekBar + AlertRangeOverlay）紧贴播放器下方 → ③ Fisheye chip scroller 占中间剩余空间 → ④ "返回列表"按钮在底部 center**——用户原话"将进度条上移至播放器下面，'返回列表'按钮下移"。倍速菜单：`PopupMenu` → `MaterialAlertDialogBuilder`——用户原话"切倍速的菜单太生硬了，优化一下UI"。新菜单有 Material 3 圆角（28dp）、淡入缩放动画、单选列表项 + 标题"播放速度"、主题感知颜色；点击即应用并 dismiss。保留 v1.6.4 rev1 的：① `PlayerFullscreenHelper.controllerSyncViews` 全屏点击 playerView toggle 插件可见性（dayScrubBarContainer + motionChipScroller + btnBack + btnPlaybackSpeed）；② `playerHostFrame` 作为 `secondaryPlayerView` 全屏时也变 MATCH_PARENT；③ 倍速 `formatSpeed` 修复 1.5x 显示成 1x 的 bug。
