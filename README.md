...
- **Fisheye chip 稀疏化 + 全屏按钮放大 + app 图标放大（v1.6.4 rev4）**：v1.6.4 rev3 的 chip "还是太多了"。rev4 三处改动：① **chip 数量减半 + 边缘压缩为细线**——`RecordingsDialog.populateMotionChips` 中 chip cap 从 200 降到 60（按 `motion_score` 取 top 60）；`FisheyeChipScroller` 增加 `textThreshold = 0.7` 阈值，当 chip 的 fisheye scale 低于 0.7 时把 `TextView.text` 清空并强制宽度为 `thinBarWidthDp=3dp`——边缘 chip 变成无文字的彩色细条，中心 chip 仍显示"HH:mm"标签。chip 的 label 通过 `tv.tag = label` 保存以便在滚动时还原。alpha 保持 1.0 不淡化。② **全屏按钮靠右下 + 放大**——`btnFullscreen` 的 `marginEnd/marginBottom` 从 6dp 减到 2dp（更靠右下角），`iconSize` 显式设为 36dp（默认 24dp），`padding` 从 8dp 加到 12dp。③ **app 图标放大**——`scripts/generate_app_icon.ps1` 自动检测 `home.png`（1024×1024）的内容边界（实际内容 477×434，原本有 ~50% 透明边距），裁剪后放到 8% padding 的画布上（v.s. 之前 ~15% padding），视觉放大 ~15%。生成 5 密度（mdpi/hdpi/xhdpi/xxhdpi/xxxhdpi）× 2 形状（方/圆）= 10 个 PNG。
