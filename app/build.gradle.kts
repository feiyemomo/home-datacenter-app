...
android {
    ...
    signingConfigs {
        ...
        // cannot overwrite a previously-installed version 閳?Android's
        // package installer reports "鐎瑰顥婇崠鍛￥閺? / "package invalid"
        ...
        // published APKs 閳?guard it carefully: losing it means you can no
    ...
    buildTypes {
        debug {
            ...
            // different keystores 閳?same package name + different signature
            // is an install conflict ("鏉烆垯娆㈤崠鍛瑢閻滅増婀佹潪顖欐閸栧懎鐡ㄩ崷銊ュ暱缁?). With this,
    ...
    // v1.5.4: packaging block 閳?keep useLegacyPackaging=false so AGP
...
dependencies {
    ...
    // WebRTC 閳?used for sub-second live streams via go2rtc's /api/v1/webrtc
    ...
    // v1.5.4: upgraded 1.1.0 閳?1.3.10 to fix Android 15+ "16 KB page size
    ...
    // JVM 閳?needed so JwtUtil (which parses JWT payloads with JSONObject)
